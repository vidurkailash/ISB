	...
	while (mySpec.getRTime() != 0) {
		...
		for (int z = 0; z < mySpec.size(); z++) {
			...
			for (int i = 0; i < my_peptide_lists.d_list.size(); i++) {
				...
				if (((my_peptide_lists.d_list[i].d_pep_seq_rt / 60) <= mySpec.getRTime() + 2) && ((my_peptide_lists.d_list[i].d_pep_seq_rt / 60) >= mySpec.getRTime() - 2) && (my_peptide_lists.d_list[i].d_pep_seq_mz <= mySpec[z].mz + 0.01) && (my_peptide_lists.d_list[i].d_pep_seq_mz >= mySpec[z].mz - 0.01)) {

					my_match_lists.results1.push_back(my_markers()); //input 
					/*cout << "-------------" << mySpec.getRTime() << "-------------" << endl;*/
					my_match_lists.results1[l].spec_rt = mySpec.getRTime(); //input
					/*cout << "-------------" << mySpec.getScanNumber() << "----------" << mySpec.size() << "----------" << endl;*/
					my_match_lists.results1[l].spec_sn = mySpec.getScanNumber(); //input
					my_match_lists.results1[l].spec_size = mySpec.size(); //input
					/*cout << mySpec[z].mz << "\t" << mySpec[z].intensity << endl;
					cout << my_peptide_lists.d_list[i].rtime / 60 << "   " << my_peptide_lists.d_list[i].mz << endl;*/
					my_match_lists.results1[l].spec_mz = mySpec[z].mz; //input
					my_match_lists.results1[l].spec_intensity = mySpec[z].intensity; //input
