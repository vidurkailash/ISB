...
void deep_functions::print(peptide_lists& my_peptide_lists, metrics& my_metrics) {
    ...
    if (my_peptide_lists.peptide_matches.size() < 20) {
        for (size_t i = 0; i < my_peptide_lists.peptide_matches.size(); i++) {
            cout << i+1 << ": " << " misscleaved sequence: " << my_peptide_lists.peptide_matches[i].mc_pep_seq << "        misscleaved intensity: " << my_peptide_lists.peptide_matches[i].mc_areaXIC << "        tryptic sequence: " << my_peptide_lists.peptide_matches[i].ft_pep_seq << "       tryptic intensity: " << my_peptide_lists.peptide_matches[i].ft_areaXIC << "        intensity ratio: " << my_peptide_lists.peptide_matches[i].ft_areaXIC / my_peptide_lists.peptide_matches[i].mc_areaXIC << endl;
        }
    }
    else {
